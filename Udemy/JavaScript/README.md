...
```js
a && b && c && d;
// оператор вернёт значение первого ложноподобного операнда при вычислении,
// либо значение последнего операнда, если все операнды оказались истиноподобными
...
> FOREACH
> **forEach возвращаемое значение отбрасывается**
...
> MAP
> **map возвращаемое значение добавляется как один элемент в новый массив**
...
Условие         // Любое выражение
  ...
  : Выражение 2 // Если условие правдиво, тогда возвращается результат Выражения 2
```
...
```js
while (Условие) {
  // Зарезервированное слово "while"
  // Цикл выполняется пока условие правдиво
  // Блок кода, выполняемый на каждой итерации
  // Блок инструкций (Может быть ни разу не выполнен)
}
```
...
do {
  ...
  // Блок инструкций (Выполняется хотя бы один раз)
  // Блок кода, выполняемый на каждой итерации
...
Object.values(myObject).forEach((value) => {
  // Получение всех значений свойств объекта в виде массива
  console.log(value);
});
```
...
for (const prop of myObject) {
  console.log(prop); // Нельзя использовать для объектов
}
```
