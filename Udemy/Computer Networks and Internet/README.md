...
Модель TCP/IP (Transmission Control Protocol/Internet Protocol) — это набор протоколов, которые определяют, как данные передаются через сеть интернет. Она является основой для всей современной сетевой коммуникации и состоит из четырёх уровней. Каждый уровень выполняет свои задачи, работая вместе для обеспечения надёжной и эффективной передачи данных.

4 уровня модели TCP/IP

1. Уровень сетевого доступа (Network Access Layer)
   Этот уровень отвечает за физическую передачу данных по сети. Он включает в себя протоколы и технологии, которые определяют, как данные форматируются для передачи через физическую среду, такую как кабели или беспроводные соединения. Здесь работают такие протоколы, как Ethernet и Wi-Fi. Этот уровень также занимается адресацией на физическом уровне, используя MAC-адреса.

2. Межсетевой уровень (Internet Layer)
   Межсетевой уровень отвечает за маршрутизацию и адресацию данных между различными сетями. Основным протоколом этого уровня является IP (Internet Protocol). IP-адреса используются для уникальной идентификации устройств в сети, а маршрутизаторы на этом уровне определяют лучший путь для доставки данных от источника к получателю. Именно здесь происходит объединение различных сетей в единый интернет.

3. Транспортный уровень (Transport Layer)
   На этом уровне происходит сегментация данных и их надёжная доставка. Два основных протокола здесь — это TCP (Transmission Control Protocol) и UDP (User Datagram Protocol).
   TCP обеспечивает надёжную, упорядоченную и контролируемую доставку данных. Он устанавливает соединение между отправителем и получателем, разбивает данные на сегменты, нумерует их и подтверждает получение. Если пакет потерян, TCP запрашивает его повторную отправку. Это используется для таких сервисов, как веб-браузинг и электронная почта.
   UDP предлагает более простой и быстрый способ передачи без установки соединения и гарантий доставки. Он используется для приложений, где скорость важнее надёжности, например, для потокового видео или онлайн-игр.

4. Прикладной уровень (Application Layer)
   Прикладной уровень — это самый верхний уровень, с которым непосредственно взаимодействуют пользовательские приложения. Здесь работают протоколы, которые позволяют приложениям обмениваться данными. Примеры протоколов этого уровня включают:
...
- DNS (для преобразования доменных имен в IP-адреса)
  Этот уровень предоставляет сервисы, которые мы используем каждый день, от просмотра веб-сайтов до отправки сообщений.
...
NAT (Network Address Translation) используется для:
...
- Позволяет преодолеть нехватку публичных IP-адресов
- Легко развернуть и использовать
- Скрывает структуру сети от внешнего мира

**Недостатки NAT:**

- Нарушение фундаментального принципа построения IP-сетей: каждый компьютер может соединиться с любым другим компьютером
- Нет возможности подключиться к компьютерам во внутренней сети из внешного мира
- Плохо работает протоколы не устанавливающие соединения
- Некоторые прикладные протоколы работает неправильно (FTP)
...
- Порт 80 --> Внутренний адрес Web-сервиса и порт 80
...
- Позволяет установливать соединение с компьютерами во внутренней сети
- RFC 3489 и другие варианты
- Используется в VoIP, VPN и других приложениях (Skype)
...
- Частично решает проблему нехватки IP-адресов
- Легко развернуть и использовать
...
- Плохо работает многие сетевые протоколы (FTP, Skype и др.)
...
1. **IP-адрес** - **IP address**: Уникальный адрес устройства в сети.
...
## Зачем нужны доменные имена - Why Domain Names are Needed?
...
## Уровни доменных имен - Domain Name Levels
