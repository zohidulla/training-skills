...
import {
  CreatePayzeTransactionDto,
  UpdateTransactionRepoDto,
} from "@/dtos/payze-transaction.dto";
...
class PayzeTransactionRepo {
  ...
  public create = async (
    data: CreatePayzeTransactionDto
  ...
  public getByTransactionId = async (
    ...
    return this.payzeTransactionModel
      .findOne({ transactionId: transactionId })
      .lean();
  };
  public updateByTransactionId = async (
    transactionId: string,
    update: Partial<UpdateTransactionRepoDto>
  ): Promise<void> => {
    await this.payzeTransactionModel.findOneAndUpdate(
