...
class PayzeTransactionService {
  ...
  public hook = async (
    ...
    const payzeTransaction = await this.payzeTransactionRepo.getByTransactionId(
      transactionId
    );
    if (!payzeTransaction)
      throw new NotFoundError(ErrorCode.PayzeTransactionNotFound);

    switch (status) {
      case PayzeTransactionStatus.Draft: {
        const transaction = await this.payzeTransactionRepo.getByTransactionId(
          transactionId
        );
        if (!transaction) {
      ...
      case PayzeTransactionStatus.Captured: {
        ...
        await this.updatePayzeTransaction(transactionId, {
          status: status,
          cardMask: cardMask,
          performTime: performTime || Date.now(),
        });
        break;
      }
      ...
      case PayzeTransactionStatus.Blocked: {
        ...
        await this.updatePayzeTransaction(transactionId, {
          status: status,
          blockedTime: blockedTime || Date.now(),
          cardMask: cardMask,
        });
        break;
      }
  ...
  private updateOrder = async (
    orderId: string,
    data: { status: OrderStatus; performedAt?: Date; canceledAt?: Date }
  ): Promise<void> => {
    await this.orderRepo.updateById(orderId, data);
  };

  private updatePayzeTransaction = async (
